...
const PreviousYearPapersSubjectWise = () => {
  ...
  // Filtered subjects based on search input
  const filteredSubjects = subjects.filter(subject =>
    subject.toLowerCase().includes(searchTerm.toLowerCase())
  );

  // Subjects to display based on toggle
  const displayedSubjects = isOpen ? filteredSubjects : filteredSubjects.slice(0, 6);

  return (
    <section className="py-16 bg-blue-950">
      <div className="container mx-auto px-6">
        <h2 className="text-4xl font-semibold text-center text-white mb-8">
          Interview Question - Subject-wise
        </h2>
        ...
        <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {displayedSubjects.length > 0 ? (
            displayedSubjects.map((subject, index) => (
              ...
              >
                ...
                <p className="text-gray-600 mt-4">
                  Access previous year papers for {subject} to boost your exam preparation.
                </p>
                <button
            ...
            <p className="text-white col-span-full text-center">No subjects found.</p>
          )}
        </div>
